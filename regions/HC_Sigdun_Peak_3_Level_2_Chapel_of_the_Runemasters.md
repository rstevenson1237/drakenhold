...
**One thing the drawing found and did not resolve.** `HC.11` says the survey line is a Runemaster route that *bypassed the trials*; the excavation as actually cut breaks into `HD.4`, the circuit itself, which bypasses nothing. Both are true and the discrepancy is in character — Vekkut cannot read the survey he is following and is off it. **Where the line was meant to arrive is a question, not an assumption, and it is batched rather than answered.**
