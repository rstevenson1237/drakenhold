...
**Features:** Three waystones spaced along the trail, bilingual in runic and trade tongue, which together teach the script to anyone who stops at all three — the region's principal work, and the reason nothing later has to stop and explain itself. Beyond them, a set of small permanent choices made cheaply: what to clear, what to walk around, what to leave alone, and whether to step over a line that nothing is stopping anyone stepping over. Each is held at the location that offers it, and none of them is reversible in an afternoon.

**Dangers:** Low, avoidable, and all of it found by leaving the road. The trail itself answers nothing: a party that keeps to it walks the region end to end, and the d10 is generous for that reason. Everything that can hurt them announces itself first, and the three things that do are held at the three locations that hold them.

**Creatures:** Three, one per off-road hazard, and all three are avoidable and none of them holds a grudge: `Giant Spider` at `B.10`, `Wood Sprite` at `B.12`, `Goblin` at `B.6`. Hobgoblin scouts pass through at distance without stopping, pricing the toll a party has not reached yet. Nothing in this region hunts anyone, and nothing in it follows a party out.

**Secrets:** That the road was one arm of a network with places on it nobody alive has heard of, and that the network's own account of its last year survives in two ledgers counted at opposite ends of it. That the roadway is not the only dwarven work under this wood — something runs beneath it, and it is still open.

**Treasure:** Little, and that is deliberate. The region's value is knowledge and safe passage, and its one cache of portable goods sits under the one thing here that can kill outright. What is otherwise recoverable is equipment rather than wealth — useful, heavy, and worth exactly nothing to a buyer, which is a distinction the hold will make constantly and which is taught here first.
...
* **The Roadway Underfoot:** cut blocks under a generation of leaf mould, still level, still draining. The first of four places the region shows a party dwarven engineering before anything asks them to care about it.
* **The Last Sight of the Palisade:** a party that turns at the right moment sees the town from outside for the only time in the module — timber on stolen stone, small, and the only lit thing in a very large wood.
* **Wild Procedure, Taught Cheaply:** this is where Navigate, Explore, Track or Evade and Rest in the wilderness get their first outing. The trail is generous and the d10 is generous with it. Nothing here punishes a party for learning the moves on the way through.
* **Exits:** the landing gate, half a day south-east -> `A.1` · the first waystone, north-west up the trail -> `B.2`
...
* **The Bilingual Text:** the same three lines twice, runic above and trade tongue below, with the numbers as tally strokes in both. This is the module's entire teaching method for the script and it costs a party nothing but the decision to stop. Three stones is enough for a table to build its own glossary.
* **The Water Gate:** the stone names Thornhaven *Ostgir* and points back down the road at it. A party that read the inverted lintel beside the town gate has already met those two roots and gets the recognition here rather than the information — which is the better half of the trade.
* **The Reckoning:** two leagues by the stone, half a day by the walking. Compared against `B.1`, it tells a table what a *lath* is worth, and every distance on the other two stones becomes readable.
* **Exits:** the trailhead, south-east -> `B.1` · the wheel-rut reach, north-west -> `B.3` · Fenn's line where it runs alongside the trail, west -> `B.13`
...
* **The Ruts:** a finger deep in dressed stone. The arithmetic is available to anyone who wants it — the traffic that did this ran for centuries, and it stopped inside living memory. This is the first argument the module makes for taking dwarven work seriously, and it makes it without a word of narration.
* **The Gutter:** still doing its job with nobody left to maintain it. It is the same detail as the well at `A.2` and the landing courses at `A.8`, and it is the reason the culvert ahead is worth clearing rather than fording: dwarven drainage works when the silt is out of it.
* **The Noise:** hard surface, and a party crossing it is audible further than it is used to. Goblins know this reach and do not toll on it, because there is nowhere here to spring from.
* **Exits:** the first waystone, south-east -> `B.2` · the silted culvert, north-west where the roadway crosses the stream -> `B.4`
...
* **Clearing It:** an afternoon's work with tools and hands, and the water drops within the hour. It is permanent, it is cheap, and it gives the party a dry crossing for the rest of the campaign — including for anyone hauling weight back down this road later, which is the version of this that matters and which nobody will think of now.
* **Leaving It:** a chokepoint, and the goblins already treat it as one. A party wading forty paces of standing water is slow, loud, and looking down.
* **The Churned Banks:** unshod feet, many, both directions, west. Readable with no roll. The goblins water here because the stream is spread and slow, which is a fact about goblins and not about the road.
* **The Drain Mouth:** under the silt, in the culvert's upstream wall, a second opening that is not part of the crossing. It shows only to a party that clears the culvert or goes into the water deliberately — **hidden** -> `B.4a`
* **Exits:** the wheel-rut reach, south-east -> `B.3` · the second waystone, north-west -> `B.5`
...
* **The Moving Air:** the cue, and it is available before the hole is. Cold air coming out of a silted culvert on a still day is the whole clue, and a party that clears the crossing gets it for free.
* **What It Costs:** two watches in the dark, in a passage that will not let a party stand up or turn around quickly, with no water fit to drink and no second exit. **It is not slower than the road — it is faster**, and everything it costs is paid in light, air and nerve rather than in time. It skips the deadfall, the turning, the third waystone, the sprite hollow and the last rise — the northern half of the region and everything the region was going to teach — and delivers a party to `D` early, unmet, unpractised, and burning torches they will want later.
* **The Missing Grating:** the rebate is empty and the fixings are sheared rather than rusted. Something took it out, from this side, and the module does not say what. **This is a local mystery and not a setting-wide one** — it belongs to this culvert and to whatever `D.6` turns out to hold at the far end, and it is not evidence of anything larger. Referees should feel free to answer it or to leave it standing.
* **Exits:** the culvert above — **hidden** -> `B.4` · the outfall below the near bank at the river crossing — **hidden**, two watches in the dark -> `D.6`
...
* **Thurgan and Azost:** two destinations, twenty-six and forty leagues north, and nobody alive in Thornhaven has heard of either. The waystation ledger at `B.11` confirms carts went to both, with tonnage and cargo and no names. **Nothing else in the module answers them, and nothing is meant to** — they are a road network that outlived its own memory, and the correct Referee answer to "what is Azost" is that the party is holding everything anyone knows.
* **The Damage:** the crown of the stone struck off in one blow and never touched again. What sat up there was the mark of the authority that cut and kept the road — the same place the other two stones carry theirs, and `B.2` and `B.8` still have both of theirs. Whoever did this wanted the mark gone and left every distance and every destination perfectly legible, which is a choice and is readable as one. **The mark is the general device of the authority that kept this road, and that authority does not exist any more** — not a clan, not a crown, not a name anybody in the module will hand back to the party. Nothing in the hold answers it. It sits with the ruts, the gutter and the ledger as one more thing that was maintained by an institution and is now simply weather-worn and knocked about: the region's real subject is decay, and this is the sharpest piece of it.
* **The Old Spur:** leaves the road west at the stone, grassed over but readable as a made surface for the first two hundred yards -> `B.11`
* **Exits:** the culvert, south-east -> `B.4` · the deadfall, north-west up the trail -> `B.6` · the webbed thickets, off the road west -> `B.10` · the collapsed waystation, along the spur -> `B.11`
...
* **The Three Traces:** distinguishable with a little care — one broader and better used, one crossing wet ground, one running the long way round a rise. Reading them before committing is `Explore` work and it is worth doing, because the choice is not reversible in an afternoon.
* **The Smoke:** three threads, well apart, and the separation is the information. The goblins of the Ironwood are three camps that do not coordinate, and the trail tells a party so before the party ever meets one.
* **The Module Saying It Out Loud:** the road is safe and leaving it is not. This is the plainest statement of that in the campaign, and it is placed where the party is still cheap to teach.
* **Exits:** the deadfall, south-east -> `B.6` · the third waystone, north-west -> `B.8` · the webbed thickets, off the road south, the bypass rejoining -> `B.10` · west by three traces -> `C.1`, `C.2`, `C.3`
...
* **Kelmordun:** the payoff. The stone names the hold in the dwarven tongue and gives *Drakenhold* as the trade-tongue rendering, and it is the first time anything in the module tells a party that the place has a real name and that everyone alive is using a corruption of it. A table that stopped at the first two stones reads this one itself, out loud, before the Referee gets to the trade-tongue line — which is the entire method paying off.
* **The Three Gates:** the Wyrm Gate, the Hammer Gate and the Deep Gate, named as though a traveller would need to know which to use. A party will find exactly one standing open. The other two are a question the hold answers much later and this stone is where the question is planted.
* **The Intact Crown:** the mark `B.5` is missing is here, whole. It is the road authority's device and the road authority is thirty years gone, along with everyone who could have said whose it was. A party may copy it. Nothing anywhere will recognise it, and that is the answer rather than a gap in one.
* **Exits:** the turning, south-east -> `B.7` · the last rise, north-west -> `B.9` · the sprite hollow, off the road east -> `B.12`
...
* **The First Sight of the Peaks:** three of them, distinct, snow on the tops, and the middle one broken — the summit opened into a notch that the other two do not have. Nothing rises from any of them. Thornhaven has been saying *no smoke from the peaks* for ten winters and this is where a party sees what that sentence looks like. The campaign's establishing shot, and it is deliberately free: no roll, no cost, no encounter.
* **The Wood's Edge:** a hard line rather than a gradient. `Wood Sprite` cannot pass it and knows it, and anything that has been mimicking voices for two days stops here, loudly and with bad grace.
* **Days Behind:** the party has been under canopy long enough that open sky reads as exposure rather than relief. Everything from here to the crossing is watched from distance, and `D`'s hobgoblins are already counting.
* **Exits:** the third waystone, south-east -> `B.8` · the sprite hollow, off the road south, rejoining here -> `B.12` · the overlook, north-west where the canopy breaks -> `D.1`
...
* **The Husks:** wrapped, stored and hung, and still holding whatever their owners were carrying. Coin, gear, a rusted blade, a crew's worth of rope. This is the only real portable treasure in the region and it is guarded by the only thing here that can kill outright, which is the trade the region offers exactly once.
* **Taken Deliberately Or By Accident:** a party that comes here on purpose, in daylight, with fire and a plan, is doing something reasonable. A party that blunders in while going around the deadfall is doing the same thing at surprise distance and in the dark. The encounter is identical and the outcomes are not.
* **The Bypass, Priced:** these thickets are the answer to the deadfall that is not the deadfall. It is free of goblins, it costs this. The module never says which is the mistake.
* **The Spur Underneath:** the made surface of the waystation road runs through the webbed ground and out the far side, which is how the thickets connect to anything at all -> `B.11`
* **Exits:** the second waystone, back onto the trail south-east -> `B.5` · the turning, back onto the trail north-west -> `B.7` · the collapsed waystation, along the spur -> `B.11`
...
* **The Ledger:** dates, cargo, tonnage and destinations for the last year the road ran, and **no names anywhere** — this was a toll and weighing book, not a register of people. It confirms that carts went to Thurgan and to Azost, gives the distances and the loads, and cannot tell anyone who was driving them. It is the outside world's account of the road's last year, and it is the counterpart of the toll ledgers at `A.3`: the same traffic, counted at both ends, thinning to nothing in the same season.
* **Gear, Not Wealth:** rope, tackle, a cauldron, iron rings, a whole stable's worth of ordinary equipment nobody has needed for thirty years. Genuinely useful and genuinely not treasure, which is a distinction this region makes twice and the hold will make constantly.
* **The Well:** clean, deep, dwarven-collared. It is the only good water off the trail in the region and the goblins do not know it is here, because the thickets grew over the spur before they arrived.
* **Exits:** the second waystone, by the old spur out to the trail -> `B.5` · the webbed thickets, further along the spur, grown over its far end -> `B.10`
...
* **What It Costs:** time and direction. A party that walks into an argument with them loses a watch, then another, and can lose the way entirely — this is where a Navigate failure has the whole wood behind it and no landmark left to recover from, which is precisely why the hollow sits on the run-out to `D` rather than at the trailhead.
* **Riddles and Petty Bargains:** they prefer both to any kind of trouble. A trinket, a song, a good riddle or a name freely given buys passage and buys it reliably. They have watched this road for a long time and will trade what they have seen, which includes who has come up it recently and in what condition.
* **The Edge of the Wood:** they cannot follow past it, they know it, and a party that works this out can end the whole business by walking uphill to `B.9`. Learning that is the point of them.
* **The Borrowed Voices:** every voice they use is one they have actually heard on this road. Some of the voices are thirty years old.
* **Exits:** the third waystone, out to the trail south-east -> `B.8` · the last rise, out to the trail north-west -> `B.9`
...
* **Crossing Is a Choice:** and she treats it as one. She is not watching from a hide; she knows because the wood tells her, and she will meet a party inside her line within an hour, on foot, alone, and unarmed. What happens next depends entirely on whether they are carrying an axe.
* **The Two Approaches:** this line and the cutting ground at `A.12` are the same boundary met from opposite directions, and she watches both. A party that crossed here arrives at the cutting ground with a history; a party that stood in the fresh stumps first arrives here already knowing what the town has been doing to her.
* **The Marks Themselves:** cut and bound rather than carved or planted — the work of someone who could have made a wall and chose to make a sentence instead. A party that reads them as a warning has read them correctly. A party that reads them as an invitation to see who is home has also read them correctly, and that is the more interesting evening.
* **Exits:** the first waystone, out to the trail -> `B.2` · the cutting ground, the same boundary from the town side -> `A.12` · the grove, within her line -> `A.13`
...
- **No edge was added, moved or retyped**, for the second region running. `A` was a town and `B` is a road, and both are shapes that are fully known before the prose starts. This should not be read as the pattern for the peaks, where a level's outlines will generate doors the stubs never had.
- **The drain got a price for the first time: two watches.** `B.4a`–`D.6` was drawn at step 8 as "a crawl in the dark under the whole last stretch of the trail" and the outlines had to say how long that is. Eight hours underground, stooped, no water, no branch, no second exit — **which is faster than the road above it**, and the whole cost is paid in light, air and nerve. It skips the deadfall, the turning, the payoff waystone, the sprite hollow and the last rise, and delivers a party to `D` early and unpractised. **The edge is unchanged and correctly typed; what changed is that its cost is now stated rather than implied.**
- **The waystone crowns are new physical detail and they are not a thread.** *Ratified by direction at this pass.* `B.5`'s crown is struck off and `B.2`'s and `B.8`'s are intact, carrying the general device of the authority that cut and kept this road — **an authority that no longer exists and that nothing in the module names.** It is not the King's mark, it is not connected to the defacing inside the hold, and no later region owes it a payoff. It reads as what it is: a maintained thing, unmaintained, knocked about by thirty years of nobody. `B` is a region about decay and this is its sharpest instance.
- **The `B.1`/`B.2` distances now reconcile deliberately.** Half a day's walking against two leagues on the stone is the region telling a table what a *lath* is worth, which makes every distance on the other two stones readable. What was a wobble between two step-7 notes is now the teaching aid.
- **`B.11`'s ledger is the counterpart of `A.3`'s.** The same traffic counted at both ends of the same road, thinning to nothing in the same season, with no names in either. That pairing was implicit at step 7 and is now written at both ends — the only cross-region thread this pass added, and it costs no edge.
