...
* **What It Costs:** a full day and a night underground, in the dark, in a passage that will not let a party stand up or turn around quickly. No water fit to drink. No second exit. It skips the deadfall, the turning, the third waystone, the sprite hollow and the last rise — the entire northern half of the region and everything the region was going to teach — and delivers a party to `D` early, unmet, unpractised, and out of light.
* **The Missing Grating:** the rebate is empty and the fixings are sheared rather than rusted. Something took it out, from this side, and the module does not say what. It is the first thing in the campaign that is genuinely unexplained and it should be left that way.
* **Exits:** the culvert above — **hidden** -> `B.4` · the outfall below the near bank at the river crossing — **hidden**, and a day and a night in the dark -> `D.6`
...
* **The Intact Crown:** the mark `B.5` is missing is here, whole, and can be copied. Nothing in this region says whose it is.
...
- **The drain got a price for the first time.** `B.4a`–`D.6` was drawn at step 8 as "a crawl in the dark under the whole last stretch of the trail" and the outlines had to say how long that is: **a full day and a night underground**, stooped, no water, no branch, no second exit. That is now written at `B.4a`. It skips the deadfall, the turning, the payoff waystone, the sprite hollow and the last rise — the entire northern half of the region and everything the region exists to teach — and delivers a party to `D` early and unpractised. **The edge is unchanged and correctly typed; what changed is that its cost is now stated rather than implied.**
- **The waystone crowns are new physical detail and one of them is a thread.** `B.5`'s crown is struck off and `B.2`'s and `B.8`'s are intact, carrying the mark of the authority that cut and kept the road. **The mark is a shape and the module does not name it here.** A party can copy it and carry it. This is recorded in `OPEN_QUESTIONS.md` rather than resolved, because naming it would settle by side effect how far the defacing of the King's name reached outside the hold — which is `E`'s and `GC`'s ground, not this region's.
