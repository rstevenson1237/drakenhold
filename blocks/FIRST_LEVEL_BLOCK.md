...
- **`E.10` is where the survivors' families are named, and `E.11` is where they are buried.** *Opened at `E`'s step-9 pass.* The servants' rows outside the hold carry every household's name cut on its own doorpost, because nothing official ever recorded the underclass, and the unmarked ground past the last cut stone in the burying ground is where the rows' dead went. **The pockets in all three warrens are the people those doorposts name.** A party that copied them is carrying the only thing anybody has offered these survivors in thirty years that was not a demand — and, for the Peak 3 pocket in particular, an answer to the one standing goal they still hold. **Nothing in `E` states the connection and no peak may state it either**: it is earned by curiosity in a slum and spent underground. No edge, no mechanic, and it does not overturn *Fear 1, d4 until they have watched you once.*
