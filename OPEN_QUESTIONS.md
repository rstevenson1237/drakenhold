...
- **The naming and the calls made at `D`'s step-9 pass.** Four things were decided in place rather than found on the record, and all four are in the pass's question batch rather than in `DECISIONS.md`. **The garrison's end at `D.2`** — killed by their own people coming down the road at the Descent, two years after the courier they stopped under a confiscation order nobody was left to lift. **The toll's second number at `D.8`** — a small price a head going up and a share of the load coming down, which is the module's spine priced at a booth. **Three names** — Vashek the hobgoblin captain, Halvard Grellick the Charter's factor, and Emmet Tarr, the missing surveyor whose satchel is at `C.9`. **`Brynost`** — the crossing's dwarven name, compounded from *bryn* and *ost*, both already on the record. *Nothing here amends a closed region; the `C.9` satchel is described from `D`'s side only.*
