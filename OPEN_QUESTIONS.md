...
- **`D.6`, the drain outfall.** `J.9`–`D.6` is drawn one-way outward from `J`'s side, and `B.4a` now arrives there too, two watches in the dark, past a grating whose fixings were sheared from the tunnel side. Whether `D.6` should carry a referee-facing line about what else comes out of that culvert is **proposed and batched**; `D` is closed and was not amended. **Direction at `B`'s pass: leave it open, and keep it small.** This is a localised mystery belonging to one culvert and its two arrivals — not a setting-wide one, not evidence of anything larger, and a Referee may answer it or leave it standing. `D`'s own pass may write the line or decline to.
