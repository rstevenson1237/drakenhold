...
## OPEN — RAISED BY THE STRIKE TOOLING

*The pass that wrote `scripts/strip.py` and `M12`, and tried to repoint `M3`. **Route `E` both** — each is a convention the corpus has been using two ways without anyone choosing, and neither resolves by a pass arriving.*

- ~~**What `->` means.**~~ *Answered by direction: **`->` always means an exit**, ratified in `DECISIONS.md`. `B.6`'s Way Around feature keeps its sentence and loses the arrow — the prose already told the player where the bypass runs, and `B.5`→`B.10`→`B.7` carry it. **`M3` is repointed** onto the feature pointers and now passes on both sides of the strike. The question as it stood:* A feature pointer is read as an exit — `M3` would take the pointer set as the region's edge set once the `Connections:` field goes. **`B.6` uses it for something else**: its *Way Around* feature carries `-> `B.10`` while describing a bypass that runs `B.5`→`B.10`→`B.7`, around the deadfall and not out of it. The diagram agrees with the Connections field and neither draws `B.6`–`B.10`; the feature is **naming where the bypass runs, not offering it**. One instance in 195 pointers, and it is good writing — the party learns at the deadfall that there is a way around. *Alternatives: `->` always means an exit, and `B.6`'s mention loses the arrow while keeping the sentence; or the exit set is only the pointers inside a feature that is itself an exit, which needs a marker the corpus does not have; or `M3` keeps reading a field that step 12 is supposed to remove, which puts the two back in deadlock.* **Not to be resolved by assumption — a wrong call here silently adds or drops an edge at the one step that cannot be re-run.**
- ~~**Architect register survives the strike in twenty-two region files.**~~ *Answered by direction: **step 12 strikes it**, sentence by sentence rather than by the line, because in several files the procedural sentences share a line with referee-facing content. Block cross-references are kept — the blocks are published parts now, so they point a reader somewhere real. Residual architect register in the closed five after a strike: none. The finding as it stood:* Every region file carries italic process prose under its headings — *"Procedure step 7 for the code, name and three thematic tags; procedure step 9 for the outlines below them… the step-7 working notes have been absorbed"* and *"Procedure step 8. Drawn from the stubs, before the location outlines."* **`CLAUDE.md` says architect register must not survive**, and step 12's strike list does not name these, so they currently reach a reader. *Alternatives: name them in the strike list and cut them, which is the reading of the rule; keep them as a genuine note to a Referee about how the diagram is to be trusted, which is what the second one half is; or rewrite the useful half into referee register at step 10 and strike the rest.* **The `Connections:` sentence in the second one is self-invalidating** — it describes a field the same step removes.

- **The unnamed-fill descriptions need a home before the strike cuts them.** *Owner: **step 10**, per region. Route `B`, and it is named in step 12's gate.* The room-budget note — *"40 rooms budgeted; **20 are stubbed as locations** and the balance is unnamed fill inside the stated groupings — emptied bins, collapsed rank ends, sleeping holes, stores that held nothing worth taking thirty years ago and hold less now"* — is **architect register and is struck whole**, by direction. The count goes and so does the fill description sharing its line, because *"the balance is unnamed fill"* is a fragment once the count is gone and repairing it would be a rewrite rather than a removal. **The fill description is referee-facing and must not be lost**: it tells a Referee how to improvise the rooms between the stubs, which in a module built on negative space is not decoration. **It belongs in the region's `Layout:` field**, in referee register, written there at step 10 before the strike cuts the note. Sixteen region files carry one. **J2's rule governs it — the destination exists before the clause is cut** — and the strike is the cut.
