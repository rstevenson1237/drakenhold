...
- **Ashen is a woman, and the House rod is carried by one of the two crew who read runic.** **Route `E`** — it is a call rather than a gap, the content already exists at `E.7`, and ratifying it is all that is left. *Raised in the route-A sweep's batch.* Written at `E.7`, unratified, and recorded here because the Creatures field's wording — *one carrying a House rod he cannot identify* — was preserved exactly and a later pass should not undo that by accident.
