...
- **The waystone crown mark.** *Opened at `B`'s step-9 pass.* `B.5`'s crown is struck off; `B.2`'s and `B.8`'s are intact and carry the mark of the authority that cut and kept the road. The outlines write it as **a shape and not a name**, copyable and carryable, and resolve nothing. Naming it settles by side effect how far the striking of the King's name reached beyond the hold — a public surface, a day's walk from a human town, defaced by somebody. **Whose mark, and whether the defacing is the same defacing, is `E`'s or `GC`'s to answer, or to decline.** No edge, no stub, and `B` promises no payoff.
