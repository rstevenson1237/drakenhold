...
Engineer step 9, location outlines: **`A` Thornhaven, `B` Ironwood Trail, `C` Goblin Camps and `D` River Crossing are closed. Eighteen regions to go.**
...
**Suggested region order**, matching how the stubs and diagrams were built: approach `A`–`E`, then `FA`/`GA`/`HA` as the first-level block, then peak by peak `F`, `G`, `H`, then `I` and `J` last. **`A`, `B`, `C` and `D` are done; `E` Girkel is next in that order, and it closes the approach.** What `E` inherits from `D` is written at the foot of `D`'s diagram section, not here.
