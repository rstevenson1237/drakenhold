...
Engineer step 9, location outlines: **`A` Thornhaven and `B` Ironwood Trail are closed. Twenty regions to go.**
...
**Suggested region order**, matching how the stubs and diagrams were built: approach `A`–`E`, then `FA`/`GA`/`HA` as the first-level block, then peak by peak `F`, `G`, `H`, then `I` and `J` last. **`A` and `B` are done; `C` is next in that order.**
