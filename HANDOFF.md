...
Engineer step 9, location outlines: **`A` Thornhaven, `B` Ironwood Trail and `C` Goblin Camps are closed. Nineteen regions to go.**
...
**Suggested region order**, matching how the stubs and diagrams were built: approach `A`–`E`, then `FA`/`GA`/`HA` as the first-level block, then peak by peak `F`, `G`, `H`, then `I` and `J` last. **`A`, `B` and `C` are done; `D` is next in that order.**
